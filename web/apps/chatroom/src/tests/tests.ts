...
import {Status} from '../chat/state';
const SparkMD5 = require("spark-md5");


const websocket_dir = UserConfig.websocketDir;

let socket_chat: WebSocket;
let socket_file: WebSocket;
...
function start() {
    protobuf.load('../proto/sparray.proto').then((root: any) => {
            ...
            socket_chat = new WebSocket(`ws://${websocket_dir}/tests/ws`);
            socket_chat.binaryType = 'arraybuffer'
            // socket_chat = new WebSocket(`ws://${websocket_dir}/chat/123/ws`);
            // socket_file.binaryType = 'arraybuffer'

            socket_chat.onopen = (e: Event) => {
                socket_chat.send(chat_buffer);
            };
            ...
            const media_event_send = (event: any) => {
                ...
                socket_chat.send(controlBuffer);
            }

            socket_chat.onmessage = async (event: MessageEvent) => {
            ...
            socket_chat.onclose = function (event: CloseEvent) {
                if (event.wasClean) {
            ...
            socket_chat.onerror = function (error: any) {
...
uploadSliceElement.onchange = async () => {
    ...
    await uploadBigFile(dataRes, socket_chat);

}


async function uploadBigFile(dataRes: any, socket: any) {
    ...
    for (currentChunk = 0; currentChunk < totalChunks; currentChunk++) {
        ...
        socket_chat.send(ChatProto.encode(chunkmessage).finish());
    }
}
